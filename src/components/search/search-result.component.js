      ...
      (response) => {
        if (response != null && response.data.success != null) {
          ...
          const uniqueCollectionNames = [
            ...new Set(results.map((option) => option._index)),
          ];
    ...
    <Container style={{ marginLeft: "10%" }}>
      ...
      <div className="searchMainContainer">
        {finishedMappingObjects && (
          <FilterWrapper
            searchResultTypes={collections}
            setFinalFilter={changeFinalFilter}
            isLoading={isLoading}
            isLoadingDataFinished={isLoadingDataFinished}
          ></FilterWrapper>
        )}
        <div className="cardsList">
          {hitsCount != null && (
            <h2 className="searchFont">Search results count: {hitsCount}</h2>
          )}
          {(function () {
                ...
                if (option._index === "story") {
                  ...
                  return (
                    <SoftwareApplicationCard
                      key={index}
                      softwareapplication={option._source}
                    ></SoftwareApplicationCard>
                  );
                  ...
                  return (
                    <CommentCard
                      key={index}
                      comment={option._source}
                    ></CommentCard>
                  );
                } else if (option._index == "user") {
                  return (
                    <UserCard key={index} user={option._source}></UserCard>
                  );
                }
              });
            }
