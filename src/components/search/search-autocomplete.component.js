...
export default class SearchAutocomplete extends Component {
  ...
  renderCardType(option) {
    if (option._index === "story") {
      ...
      return (
        <CommentCard key={option.id} comment={option._source}></CommentCard>
      );
    } else if (option._index == "user") {
      ...
      <Container style={{ display: "inline-flex" }}>
        <AsyncTypeahead
          ...
          renderMenuItemChildren={(option) => (
            <section>{this.renderCardType(option)}</section>
          )}
        />
        <Button variant="outline-success">
          {" "}
          <a href={`/search-result/${this.state.selected}`}>Search</a>
        </Button>
      </Container>
    );
  }
