...
export const FilterWrapper = (props) => {
  ...
  useEffect(() => {
    ...
    if (softwareApplicationFilter) {
      finalFilter = {
        ...finalFilter,
        filterSoftwareApplicationDto: softwareApplicationFilter,
      };
    }
    if (userFilter) {
    ...
    storyFilter,
    storyTaskFilter,
    softwareApplicationFilter,
    userFilter,
    commentFilter,
  ]);

  return !isLoading && isLoadingDataFinished ? (
    !searchResultTypes || searchResultTypes.length === 0 ? (
      <div>
        <h2>Could not load any filters</h2>
      </div>
    ) : (
      <div className="sidenav">
        {searchResultTypes.includes("story") ? (
          <StoryFilter
            setStoryFilter={setStoryFilter}
            keepFormData={keepFormData}
            setKeepFormData={setKeepFormData}
          ></StoryFilter>
        ) : (
          <></>
        )}
        {searchResultTypes.includes("storytask") ? (
          <StoryTaskFilter
            setStoryTaskFilter={setStoryTaskFilter}
            keepFormData={keepFormData}
            setKeepFormData={setKeepFormData}
          ></StoryTaskFilter>
        ) : (
        ...
        {searchResultTypes.includes("softwareapplication") ? (
          <SoftwareApplicationFilter
            setSoftwareApplicationFilter={setSoftwareApplicationFilter}
            keepFormData={keepFormData}
            setKeepFormData={setKeepFormData}
          ></SoftwareApplicationFilter>
        ) : (
        ...
        {searchResultTypes.includes("user") ? (
          <UserFilter
        ...
        {searchResultTypes.includes("comment") ? (
          <CommentFilter
