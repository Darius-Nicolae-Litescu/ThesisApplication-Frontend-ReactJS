...
export const StoryTaskFilter = (props) => {
  ...
  const getFilterValues = ({ formData }) => {
    let filterStoryTaskDto = new FilterStoryTaskDto(formData.title, formData.description, formData.storyPoints, formData.createdByUsername, formData.assignedToUsername, formData.status);
  ...
  return (
    <div className="StoryTaskFilter">
      <Form schema={storyTaskSchema} formData={keepFormDataLogic()} onSubmit={getFilterValues} >
