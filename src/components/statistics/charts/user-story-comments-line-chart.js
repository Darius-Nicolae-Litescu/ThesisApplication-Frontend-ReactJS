...
import { FetchCountUserStoryCommentsData } from "../../hooks/statistics/fetch-count-user-story-comments";

export default function UserStoryCommentsLineChart() {
  const {
    status,
    data: countUserStoryCommentsData,
    error,
  } = FetchCountUserStoryCommentsData();
  const [countUserStoryCommentsChartData, setCountUserStoryCommentsChartData] =
    useState();
  ...
  useEffect(() => {
    if (
      ...
      countUserStoryCommentsData.length > 0 &&
      !countUserStoryCommentsChartData
    ) {
