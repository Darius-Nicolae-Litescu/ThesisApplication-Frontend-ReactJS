...
import { FetchCountStoryCommentsData } from "../../hooks/statistics/fetch-count-story-comments";

export default function UserStoryCommentsDoughnutChart() {
  const {
    status,
    data: countStoryCommentsData,
    error,
  } = FetchCountStoryCommentsData();
  const [countStoryCommentsChartData, setCountStoryCommentsChartData] =
    useState();
  ...
  useEffect(() => {
    if (
      ...
      countStoryCommentsData.length > 0 &&
      !countStoryCommentsChartData
    ) {
