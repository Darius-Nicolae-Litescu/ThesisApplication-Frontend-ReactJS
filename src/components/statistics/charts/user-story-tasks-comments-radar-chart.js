...
export default function UserStoryCommentsRadarChart() {
  const {
    status,
    data: countUserStoryTaskCommentsData,
    error,
  } = FetchCountUserStoryTaskCommentsData();
  const [
    countUserStoryTaskCommentsChartData,
    setCountUserStoryTaskCommentsChartData,
  ] = useState();
  ...
  const processUserStoryTaskCommentsChartData = () => {
    ...
    setCountUserStoryTaskCommentsChartData(
      countUserStoryCommentsChartFinalData
    );
  };
  ...
  useEffect(() => {
    if (
      ...
      countUserStoryTaskCommentsData.length > 0 &&
      !countUserStoryTaskCommentsChartData
    ) {
