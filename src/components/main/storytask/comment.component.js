        ...
        >
          {attachmentResponseDto.map((attachment, index) => (
            <div key={attachment.id} className="attachment">
              <a
                target="_blank"
                href={"download-file/" + attachment.name}
                onClick={() => downloadFile(attachment.url)}
                style={{ margin: "12px 36px 15px 0px" }}
              >
                <div style={{ width: "50px" }}>
                  <FileIcon
                    extension={regexForFileExtension.exec(attachment.name)[1]}
                    {...defaultStyles.docx}
                  />
                </div>
                <div className="bottom-container">
                  <h2 className="date-attachment-moment-format">
                    {attachment.name.length > 30
                      ? attachment.name.substring(0, 27) + "..."
                      : attachment.name}
                  </h2>
                  <Moment
                    style={{ fontSize: "15px", font: "caption" }}
                    className="mt-0 mb-1 text-muted"
                    format="YYYY-MM-DD HH:mm"
                  >
                    {attachment.postedAt}
