...
export default function DynamicTable(props) {
  ...
  const handleDelete = (selectedCategory, rowId) => {
    if (selectedCategory === "SoftwareApplication") {
      SoftwareApplicationService.deleteSoftwareApplication(rowId)
        .then((response) => {
          if (response != null) {
      ...
      CategoryService.deleteCategory(rowId)
        .then((response) => {
          if (response != null) {
            setData(response.data);
          }
      ...
      StoryService.deleteStory(rowId)
        .then((response) => {
          if (response != null) {
            setData(response.data);
          }
      ...
      StoryTaskService.deleteStoryTask(rowId)
        .then((response) => {
          if (response != null) {
            setData(response.data);
          }
      ...
      PersonService.deletePerson(rowId)
        .then((response) => {
          if (response != null) {
            setData(response.data);
          }
      ...
      PositionService.deletePosition(rowId)
        .then((response) => {
          if (response != null) {
            setData(response.data);
          }
  ...
  useEffect(() => {
    if (
      selectedCategory === "SoftwareApplication" &&
      selectedOperation === "insert-update-delete"
    ) {
  ...
  if (!data || !columnNames) {
    return (
      <div>Loading {camelCaseToTitle(selectedCategory)} data, please wait!</div>
    );
  }
  ...
  return (
    <div className="GenericTable">
      <Button
        ...
        className="btn btn-danger btn-xs"
        onClick={() => {
          handleInsertOrUpdateClick(selectedCategory, null);
        }}
      >
        Insert
      </Button>
