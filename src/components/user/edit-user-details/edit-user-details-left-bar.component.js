...
const optionsData = [
    { key: 'GeneralInformationSection', value: 'General information', options: generalInformationOptionsData },
    { key: 'SecuritySection', value: 'Security', options: securityOptionsData },
    { key: 'DeleteAccountSection', value: 'Delete account', options: deleteAccountOptionsData },
];


export default function EditUserLeftBar(props) {
    const {setShowChangeEmailComponent, setShowChangePasswordComponent, setShowChangeProfilePictureComponent} = props;
    ...
    return (
        <nav className="d-none d-md-block col-sm-2 col-md-2 col-lg-2 col-xl-2 sidebar">
            <div className="sidebar-sticky">
                <ListGroup style={{ marginLeft:"5px", marginTop: "50%", backgroundColor: "#e9ecef", borderRadius:"25px"}}>
                    <ListGroup.Item style={{ borderRadius:"25px", display: "grid", textAlign: "right", justifyItems: "end", textAlignLast: "end", backgroundColor:"white", color:"black" }}>
                        {optionsData.map((item) => {
                            return <DropdownButton
                                variant="Secondary"
                                className="nav-item dropleft"
                                style={{marginTop: "5px", marginBottom:"5px", borderRadius:"25px", marginLeft: "10%", width: "100%", backgroundColor: "white", color: "black" }}
                                as={ButtonGroup}
                                key={item.value}
                                ...
                                {item.options.map((itemOptions) => {
                                    return <Dropdown.Item style = {{borderRadius:"25px"}} key={itemOptions.value} onClick={() => onItemClick(item.key, itemOptions.key)}>
                                        {itemOptions.value}
                                    </Dropdown.Item>
                                })}
                            </DropdownButton>
